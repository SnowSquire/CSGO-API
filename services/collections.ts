...
function parseItemSelfOpening(
    ...
    return {
        ...
        contains: (skinsByCollections?.[item.name] ?? []).map(i => {
            if (i.name instanceof Object) {
                throw Error("Unexpected object name in self-opening collection");
            }
            return {
                ...i,
                name: $t(i.name, false, languageResource),
...
export function getCollections(
    ...
    const collections = [
        ...itemSets.filter(isCollection).map(item => parseItem(item, state, languageResource)),
        ...Object.values(items)
            .filter(isSelfOpeningCollection)
            .map(item => parseItemSelfOpening(item, state, languageResource)),
